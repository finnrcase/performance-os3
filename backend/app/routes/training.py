...
from backend.app.core.database import load_recent_rows
from backend.app.utils.helpers import to_float, to_int
...
def get_training_summary(window: str = "weekly", period: str = "all") -> dict:
    return {"status": "deferred", "window": window, "period": period, "items": [], "summary": {}, "message": "Training summaries are deferred in the clean backend."}


@router.get("/api/training/summary/status")
def get_training_summary_status() -> dict:
    return {"status": "idle", "cache_ready": False, "message": "Local training cache reads are enabled; summary generation is deferred."}


@router.get("/api/training/strength-trends")
def get_strength_trends() -> dict:
    return {"status": "deferred", "items": [], "summary": "Strength trends are deferred."}


@router.get("/api/training/sync/hevy/status")
def get_hevy_sync_status() -> dict:
    return {"status": "disabled", "configured": False, "running": False, "message": "Automatic Hevy sync is disabled in the clean backend."}


@router.post("/api/training/sync/hevy")
def sync_hevy() -> dict:
    return {"status": "disabled", "message": "Hevy sync must be re-enabled explicitly later."}


@router.post("/api/training/workout-date")
def update_workout_date(payload: dict) -> dict:
    return {"status": "deferred", "item": payload}


@router.get("/api/training/export/hevy-raw")
def export_hevy_raw() -> dict:
    return {"status": "deferred", "items": []}


@router.get("/api/training/export/normalized")
def export_normalized_training() -> dict:
    return {"status": "ok", "items": training_history_payload(limit=200, days=3650).get("items", [])}


@router.post("/api/training/consolidate-history")
def consolidate_training_history() -> dict:
    return {"status": "deferred", "raw_rows_summarized": 0, "weekly_summaries": 0, "monthly_summaries": 0}


@router.post("/api/training/import/hevy/preview")
def preview_hevy_import() -> dict:
    return {"status": "disabled", "items": [], "message": "Hevy import is disabled in the clean backend."}


@router.post("/api/training/import/hevy")
def import_hevy() -> dict:
    return {"status": "disabled", "imported": 0, "message": "Hevy import is disabled in the clean backend."}


@router.post("/api/training/ai/insights")
def training_ai_insights(payload: dict) -> dict:
    return {"status": "deferred", "exercise_name": payload.get("exercise_name"), "insights": []}


@router.post("/api/training/import/strava")
def import_strava(payload: dict | None = None) -> dict:
    return {"status": "disabled", "imported_runs": 0, "skipped_duplicates": 0, "message": "Strava import is disabled in the clean backend."}
